...
export function useFileUpload(options: UseFileUploadOptions = {}) {
  ...
  const config = useMemo(
    () => ({ ...DEFAULT_UPLOAD_CONFIG, ...options.config }),
    [options.config]
  );

  const addFiles = useCallback(
    async (newFiles: FileList | File[]) => {
      const { validFiles, invalidFiles: basicInvalidFiles } = validateFileList(
        newFiles,
        config
      );
      ...
      signatureValidationResults.forEach(({ file, signatureResult }) => {
        if (signatureResult.isValid) {
          ...
          signatureInvalidFiles.push({
            file,
            error: signatureResult.error || "Invalid file signature",
          });
        }
      });
