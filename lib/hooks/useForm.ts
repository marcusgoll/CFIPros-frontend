...
export function useForm<T extends FieldValues>({
  ...
  const getError = useCallback((field: Path<T>): string | undefined => {
  ...
  const hasError = useCallback((field: Path<T>): boolean => {
...
import { loginSchema, contactSchema, type LoginFormData, type ContactFormData } from '@/lib/validation/schemas';

// Specialized hooks for common forms with proper typing
export function useLoginForm(onSubmit?: (data: LoginFormData) => void | Promise<void>) {
  return useForm({
    schema: loginSchema,
    onSubmit,
  });
...
export function useContactForm(onSubmit?: (data: ContactFormData) => void | Promise<void>) {
  return useForm({
