...
async function sharingHandler(request: NextRequest, { params }: { params: { batchId: string } }) {
  ...
  try {
    ...
    switch (method) {
      ...
      case 'POST':
        ...
        requestBody = JSON.stringify(validationResult.data);
    ...
    const response = await proxyRequest(`/v1/batches/${batchId}/sharing`, {
      method,
    ...
    return handleAPIError(CommonErrors.INTERNAL_ERROR('Sharing service temporarily unavailable. Please try again.'));
  }
}

// Apply middleware with sharing-specific configuration
export const GET = withAPIMiddleware(sharingHandler, {
  endpoint: 'batch-sharing',
  cors: true,
  methods: ['GET', 'POST', 'DELETE', 'OPTIONS']
});

export const POST = withAPIMiddleware(sharingHandler, {
  endpoint: 'batch-sharing',
  cors: true,
  methods: ['GET', 'POST', 'DELETE', 'OPTIONS']
});

export const DELETE = withAPIMiddleware(sharingHandler, {
  endpoint: 'batch-sharing',
  cors: true,
  methods: ['GET', 'POST', 'DELETE', 'OPTIONS']
});
