...
async function auditExportHandler(request: NextRequest, { params }: { params: { batchId: string } }) {
  ...
  if (!allowedFormats.includes(format)) {
    return handleAPIError(CommonErrors.VALIDATION_ERROR(`Invalid export format. Allowed formats: ${allowedFormats.join(', ')}`));
  }

  // Validate date range if provided
  if (startDate && !Date.parse(startDate)) {
    return handleAPIError(CommonErrors.VALIDATION_ERROR('Invalid start date format'));
  }
  
  if (endDate && !Date.parse(endDate)) {
    return handleAPIError(CommonErrors.VALIDATION_ERROR('Invalid end date format'));
  }
  ...
  try {
    ...
    const response = await proxyRequest(`/v1/batches/${batchId}/audit/export?${queryParams.toString()}`, {
      method: 'GET',
      headers: {
    ...
    return handleAPIError(CommonErrors.INTERNAL_ERROR('Audit export service temporarily unavailable. Please try again.'));
  }
}

// Apply middleware with audit export-specific configuration
export const GET = withAPIMiddleware(auditExportHandler, {
