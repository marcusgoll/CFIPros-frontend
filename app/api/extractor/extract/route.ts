...
async function extractHandler(request: NextRequest) {
  ...
  if (!validation.isValid) {
    ...
    if (
      ...
      error = CommonErrors.NO_FILE_PROVIDED(
        "Please select at least one AKTR file to process"
      );
      ...
      error = CommonErrors.UNSUPPORTED_FILE_TYPE(
  ...
  try {
    ...
    return handleAPIError(
      CommonErrors.INTERNAL_ERROR(
        "Batch processing service temporarily unavailable. Please try again."
      )
    );
  }
}
