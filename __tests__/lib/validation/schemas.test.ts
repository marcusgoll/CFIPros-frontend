...
describe('registerSchema', () => {
  ...
  it('should reject if terms not accepted', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const termsError = result.error.errors.find(err => err.path.includes('terms'));
...
describe('contactSchema', () => {
  ...
  it('should reject short message', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const messageError = result.error.errors.find(err => err.path.includes('message'));
...
describe('profileSchema', () => {
  ...
  it('should reject invalid website URL', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const websiteError = result.error.errors.find(err => err.path.includes('website'));
  ...
  it('should reject long bio', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const bioError = result.error.errors.find(err => err.path.includes('bio'));
...
describe('fileUploadSchema', () => {
  ...
  it('should reject empty file array', () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      const filesError = result.error.errors.find(err => err.path.includes('files'));
  ...
  it('should reject unsupported file types', () => {
    ...
    if (!result.success) {
      const filesError = result.error.errors.find(err => err.path.includes('files'));
      expect(filesError?.message).toBe('Only JPEG, PNG, GIF, and PDF files are allowed');
    }
  });
});
