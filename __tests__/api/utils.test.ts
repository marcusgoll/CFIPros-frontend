...
describe("API Utilities", () => {
  ...
  describe("API Error Handling", () => {
    it("should create proper Problem Details format", () => {
      ...
      // Assert shape: status and content-type header
  ...
  describe("Request Validation", () => {
    it("should validate file upload requests", async () => {
      ...
      // Add valid PDF magic bytes to satisfy security checks
      const validPdfHeader = new Uint8Array([0x25, 0x50, 0x44, 0x46]); // %PDF
      const validFile = new File([validPdfHeader, " rest"], "test.pdf", {
        type: "application/pdf",
      });
      formData.append("file", validFile);

      // Minimal NextRequest-like mock supporting formData()
      const request = {
