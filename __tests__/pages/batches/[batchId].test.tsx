...
// Mock child components that perform their own data fetching to isolate page tests
jest.mock("@/components/forms/BatchSharing", () => ({
  BatchSharing: () => React.createElement("div", { "data-testid": "batch-sharing" }),
}));
jest.mock("@/components/forms/ConsentManager", () => ({
  ConsentManager: () => React.createElement("div", { "data-testid": "consent-manager" }),
}));
...
describe("Batch Status Page", () => {
  beforeEach(() => {
    ...
    // Mock successful batch status fetch by default (for all calls)
  ...
  it("displays batch status information correctly", async () => {
    ...
    // Check file statistics labels are present
  ...
  it("handles batch export correctly", async () => {
    ...
    await waitFor(() => {
      expect(mockedFetch).toHaveBeenCalledWith(
        "/api/batches/test-batch-123/export?format=pdf"
      );
    });
  });
  ...
  it("displays error state when batch fetch fails", async () => {
    ...
    expect(
      screen.getByText(/failed to fetch batch status: 404/i)
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /upload new files/i })
    ).toBeInTheDocument();
  });
  ...
  it("shows failed state with error message", async () => {
    ...
    expect(
      screen.getByText("Processing failed due to invalid file format")
    ).toBeInTheDocument();
  });
  ...
  it("polls for status updates when processing", async () => {
    ...
    // Should make at least two calls (initial + poll). Effect reruns may add another
    await waitFor(() => {
      expect((mockedFetch as jest.Mock).mock.calls.length).toBeGreaterThanOrEqual(2);
    });
