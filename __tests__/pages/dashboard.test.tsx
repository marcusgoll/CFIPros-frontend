...
jest.mock('@clerk/nextjs/server', () => ({
  currentUser: jest.fn().mockResolvedValue({ id: 'u1', firstName: 'Ace', emailAddresses: [{ emailAddress: 'ace@example.com' }] }),
}));
...
describe('Authed Dashboard', () => {
  beforeEach(() => {
    ...
    global.fetch = jest.fn().mockImplementation((url: RequestInfo) => {
      ...
      if (u.includes('/api/flashcards/dueCount')) {
        return Promise.resolve(new Response(JSON.stringify({ count: 12 }), { status: 200, headers: { 'content-type': 'application/json' } }));
      }
      if (u.includes('/api/reports')) {
        return Promise.resolve(new Response(JSON.stringify([
          { id: 'r1', date: '2024-01-02T00:00:00Z', score: 82, weakAcsCodes: ['PA.I.A.K1', 'PA.I.B.K2'] },
          { id: 'r2', date: '2024-01-01T00:00:00Z', score: null, weakAcsCodes: ['PA.III.B.K4'] },
      ...
      if (u.includes('/api/study-plans/current')) {
        ...
        ], nextLesson: { title: 'Crosswind Landings', etaMinutes: 20 } }), { status: 200, headers: { 'content-type': 'application/json' } }));
      }
      if (u.includes('/api/me')) {
        return Promise.resolve(new Response(JSON.stringify({ plan: 'free' }), { status: 200, headers: { 'content-type': 'application/json' } }));
      }
      if (u.includes('/api/billing/credits')) {
        return Promise.resolve(new Response(JSON.stringify({ plan: 'free', credits: 3 }), { status: 200, headers: { 'content-type': 'application/json' } }));
      }
      if (u.includes('/api/activity')) {
        return Promise.resolve(new Response(JSON.stringify({ items: [ { id: 'a1', message: 'Mapped AKTR', timestamp: '2024-01-01T00:00:00Z' } ] }), { status: 200, headers: { 'content-type': 'application/json' } }));
      }
  ...
  it('shows onboarding actions with correct links', async () => {
    ...
    expect(screen.getByRole('button', { name: /upload aktr/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /generate study plan/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /try a tool/i })).toBeInTheDocument();
  ...
  it('shows quick tools grid with six buttons', async () => {
    const Page = (await import('@/app/(authed)/dashboard/page')).default;
    render(await Page());
    const tools = ['AKTRACS', 'Flashcards', 'Practice Test', 'Crosswind', 'W&B', '8710 Checker'];
    tools.forEach(label => expect(screen.getByRole('button', { name: new RegExp(label, 'i') })).toBeInTheDocument());
  });
