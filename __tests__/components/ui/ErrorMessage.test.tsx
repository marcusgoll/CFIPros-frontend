...
import { ErrorMessage, FormError, PageError, InlineError } from '@/components/ui/ErrorMessage';
...
describe('ErrorMessage', () => {
  ...
  it('should apply different variant styles', () => {
    const { rerender } = render(<ErrorMessage error="Test error" variant="error" />);
    let alertElement = screen.getByRole('alert');
    expect(alertElement).toHaveClass('bg-red-50', 'border-red-200', 'text-red-800');

    rerender(<ErrorMessage error="Test error" variant="warning" />);
    alertElement = screen.getByRole('alert');
    expect(alertElement).toHaveClass('bg-yellow-50', 'border-yellow-200', 'text-yellow-800');

    rerender(<ErrorMessage error="Test error" variant="info" />);
    alertElement = screen.getByRole('alert');
    expect(alertElement).toHaveClass('bg-blue-50', 'border-blue-200', 'text-blue-800');
  });
