...
describe("FeaturesSection", () => {
  it("renders the section heading and description", () => {
    ...
    const heading = screen.getByRole("heading", { name: /Everything you need for flight training success/i });
    expect(heading).toBeInTheDocument();
    expect(heading.tagName).toBe("H2");
    
    const description = screen.getByText(/From ACS code discovery to personalized study plans/i);
  ...
  it("includes feature descriptions", () => {
    ...
    expect(screen.getByText(/SEO-optimized pages for 200\+ ACS codes/i)).toBeInTheDocument();
    expect(screen.getByText(/Upload exam reports for AI-powered/i)).toBeInTheDocument();
    expect(screen.getByText(/In-depth lesson content with progress tracking/i)).toBeInTheDocument();
    expect(screen.getByText(/AI-generated study plans tailored/i)).toBeInTheDocument();
  ...
  it("includes proper feature card links", () => {
    ...
    const acsLink = screen.getByText("ACS Code Discovery").closest("div")?.querySelector("a");
    expect(acsLink).toHaveAttribute("href", "/acs");
    
    const uploadLink = screen.getByText("Smart Document Analysis").closest("div")?.querySelector("a");
    expect(uploadLink).toHaveAttribute("href", "/upload");
    
    const premiumLink = screen.getByText("Premium Lessons").closest("div")?.querySelector("a");
  ...
  it("applies hover effects to feature cards", () => {
    ...
    const firstFeature = screen.getByText("ACS Code Discovery").closest("[class*='group']");
  ...
  it("provides proper spacing and layout structure", () => {
    ...
    const contentContainer = section?.querySelector(".mx-auto.max-w-2xl.text-center");
