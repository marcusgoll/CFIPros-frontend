...
describe("HeroSection", () => {
  ...
  it("displays trust indicator badge", () => {
    ...
    expect(
      screen.getByText(/Trusted by 10,000\+ pilots nationwide/i)
    ).toBeInTheDocument();
  });
  ...
  it("shows value proposition text", () => {
    ...
    const description = screen.getByText(
      /Comprehensive pilot training platform/i
    );
    expect(description).toBeInTheDocument();
    expect(description).toHaveTextContent(
      /SEO-discoverable ACS code references/i
    );
  ...
  it("includes background decoration element", () => {
    ...
    const section = document.querySelector("section");
    const decorationElement = section?.querySelector(
