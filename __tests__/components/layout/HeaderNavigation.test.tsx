...
describe("HeaderNavigation", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    // Provide default Clerk user state
    const { useUser } = require("@clerk/nextjs");
    if (useUser && typeof useUser.mockReturnValue === "function") {
  ...
  it("toggles mobile menu on hamburger click", async () => {
    ...
    expect(
      screen.getByLabelText("Open main menu").getAttribute("aria-expanded")
    ).toBe("false");
  ...
  it("handles mobile menu item clicks", async () => {
    ...
    // Click a menu item (within open mobile menu)
    const mobileMenu = screen.getByTestId("mobile-menu");
    const mobileMenuItem = within(mobileMenu).getByRole("link", {
      name: /research/i,
    });
  ...
  it("renders mobile accordion for nested menus", async () => {
    ...
    const mobileFeaturesAccordion =
      within(mobileMenu).getByText("Our Features");
