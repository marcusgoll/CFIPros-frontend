...
describe("ScreenshotPreview", () => {
  ...
  it("renders screenshot preview when visible", () => {
    ...
    expect(
      screen.getByAltText("File Upload feature screenshot")
    ).toBeInTheDocument();
  ...
  it("handles image loading errors", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/failed to load screenshot/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it("displays proper screenshot URL", () => {
    ...
    expect(image).toHaveAttribute(
      "src",
      "/images/features/upload-screenshot.jpg"
    );
  });
  ...
  it("handles long feature names appropriately", () => {
    ...
    expect(
      screen.getByText(
        "Very Long Feature Name That Should Be Handled Gracefully"
      )
    ).toBeInTheDocument();
  });
