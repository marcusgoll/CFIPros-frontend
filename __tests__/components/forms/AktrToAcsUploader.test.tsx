...
const mockedTrackUploadStarted = trackUploadStarted as jest.MockedFunction<typeof trackUploadStarted>;
const mockedTrackUploadCompleted = trackUploadCompleted as jest.MockedFunction<typeof trackUploadCompleted>;
const mockedTrackUploadFailed = trackUploadFailed as jest.MockedFunction<typeof trackUploadFailed>;
...
describe('AktrToAcsUploader Component', () => {
  ...
  const validPdfFile = createMockFile('aktr-report.pdf', 2 * 1024 * 1024, 'application/pdf');

  it('renders the upload interface with proper headings and instructions', () => {
    render(<AktrToAcsUploader />);
    
    // Check main heading
    expect(screen.getByRole('heading', { name: /upload knowledge test reports/i })).toBeInTheDocument();
    
    // Check file uploader is present
    expect(screen.getByRole('button', { name: /browse files/i })).toBeInTheDocument();
    
    // Check help text
    expect(screen.getByText(/what is a knowledge test report/i)).toBeInTheDocument();
    expect(screen.getByText(/airman knowledge test report/i)).toBeInTheDocument();
    
    // Check submit button is initially disabled
    const submitButton = screen.getByRole('button', { name: /analyze reports/i });
  ...
  it('enables submit button when files are selected', async () => {
    ...
    const fileInput = screen.getByLabelText(/file upload/i, { selector: 'input[type="file"]' });
    const submitButton = screen.getByRole('button', { name: /analyze reports/i });
  ...
  it('shows processing state during upload', async () => {
    ...
    expect(screen.getByRole('button', { name: /processing files/i })).toBeDisabled();
  ...
  it('updates file count display correctly', async () => {
    ...
    const secondFile = createMockFile('second-report.pdf', 1024 * 1024, 'application/pdf');
  ...
  it('provides helpful information about AKTR reports', () => {
    ...
    expect(screen.getByText(/what is a knowledge test report/i)).toBeInTheDocument();
    expect(screen.getByText(/airman knowledge test report/i)).toBeInTheDocument();
    expect(screen.getByText(/airman certification standards/i)).toBeInTheDocument();
  });
  ...
  it('shows progress for multiple files independently', async () => {
    ...
    const file2 = createMockFile('report2.pdf', 2 * 1024 * 1024, 'application/pdf');
